...
/**
 * Test helpers for the canvas question type.
 *
...
/**
 * Test helper class for the canvas question type.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_canvas_test_helper extends question_test_helper {
    public function get_test_questions() {
        return array('frogtoad', 'frogonly');
    }

    /**
     * Makes a canvas question with correct ansewer 'frog', partially
     * correct answer 'toad' and defaultmark 1. This question also has a
     * '*' match anything answer.
     * @return qtype_canvas_question
     */
    public function make_canvas_question_frogtoad() {
        question_bank::load_question_definition_classes('canvas');
        $sa = new qtype_canvas_question();
        ...
        $sa->qtype = question_bank::get_qtype('canvas');

        return $sa;
    }

    /**
     * Gets the question data for a canvas question with with correct
     * ansewer 'frog', partially correct answer 'toad' and defaultmark 1.
     * This question also has a '*' match anything answer.
     * @return stdClass
     */
    public function get_canvas_question_data_frogtoad() {
        $qdata = new stdClass();
    ...
    /**
     * Makes a canvas question with just the correct ansewer 'frog', and
     * no other answer matching.
     * @return qtype_canvas_question
     */
    public function make_canvas_question_frogonly() {
        question_bank::load_question_definition_classes('canvas');
        $sa = new qtype_canvas_question();
        test_question_maker::initialise_a_question($sa);
        $sa->name = 'Short answer question';
    ...
    /**
     * Gets the question data for a canvas questionwith just the correct
     * ansewer 'frog', and no other answer matching.
     * @return stdClass
     */
    public function get_canvas_question_data_frogonly() {
        $qdata = new stdClass();
        test_question_maker::initialise_question_data($qdata);
